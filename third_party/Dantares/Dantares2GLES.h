...
#ifndef DANTARES2_GLES_H
#define DANTARES2_GLES_H
...
{
public:
    explicit Dantares2GLES(float SquareSize, float FloorHeight, float CeilingHeight);
...
private:
    class SpaceClassGLES : public SpaceClass
    {
    public:
        explicit SpaceClassGLES(int Type) noexcept;

        SpaceClassGLES(SpaceClassGLES &&Other) noexcept;
        SpaceClassGLES &operator = (SpaceClassGLES &&Other) noexcept;
        ~SpaceClassGLES() noexcept override;

        void GenerateFaces(float SquareOffset, float FloorHeight, float CeilingHeight) override;
        void DrawFace(int FaceIndex) override;

    private:
        void MoveFrom(SpaceClassGLES &&Other) noexcept;
    };
};
