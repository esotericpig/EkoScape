...
void ArtStyles::load_gpu_gfx(const AssetMan& assets,GpuGfxLoader& gfx,bool is_weird) {
  ...
  for(const auto& asset_dir : assets.asset_dirs()) {
    ...
    try {
      ...
      const auto err_msg = TextUtil::fmt("Failed to crawl Textures folder `{}`: {}.",
                                         tex_dir.string(),e.what());
  ...
  if(texture_styles_.empty()) {
    throw CybelError{"Failed to find/load any Styled Textures in Textures folder `{}`.\n{}",
                     Assets::kTexturesSubDir.string(),errors};
  }
