...
void Map::load_metadata(TextReader& reader,std::string_view file) {
  ...
  if(!parse_header(line,data_i)) {
    throw CybelError{"Invalid header [{}] in map [{}].",line,file};
  }
  if(!kSupportedVersions.in_range(data_i)) {
    throw CybelError{"Unsupported version [{}] in map [{}].",data_i,file};
  }
  version_ = data_i;
...
void Map::load_grids(TextReader& reader,const SpaceCallback& on_space,
  ...
  if(!has_player) {
    throw CybelError{
      "Missing a Player space {{{},{},{},{}}} in a grid of map [{}].",
  ...
  if(!has_end) {
    throw CybelError{"Missing an End space [{}] in a grid of map [{}].",
                     SpaceTypes::value_of(SpaceType::kEnd),file};
  }
}
...
Map& Map::parse_grid(const std::vector<std::string>& lines,Size2i size,const SpaceCallback& on_space,
  ...
  if(grids_.size() >= Dantares2::MAXMAPS) {
    throw CybelError{"Too many grids in map [{}]; max is {}.",file,Dantares2::MAXMAPS};
  }
  ...
  if(size.w <= 0 || size.h <= 0) {
    throw CybelError{"Grid size [{}x{}] in map [{}] must at least be 1x1.",size.w,size.h,file};
  }
...
void Map::update_bridge_space(const Pos3i&,SpaceType) {}

std::string Map::build_header() const {
  return TextUtil::fmt(kHeaderFmt,version_);
}
