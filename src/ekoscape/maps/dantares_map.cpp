...
#include "ekoscape/maps/map_grid.h"
...
DantaresMap::DantaresMap(Dantares2& dantares,SetTextures set_textures)
  : dantares_{dantares},set_textures_{std::move(set_textures)} {}
...
void DantaresMap::add_to_bridge() {
  ...
  if(grid_z_ < 0 || grid_z_ >= static_cast<int>(grids_.size())) {
    throw CybelError{"Invalid grid Z [{}] for map [{}] of size [{}].",grid_z_,title_,grids_.size()};
  }
  if(player_init_pos_.z < 0 || player_init_pos_.z >= static_cast<int>(grids_.size())) {
    throw CybelError{"Invalid player Z [{}] for map [{}] of size [{}].",
                     player_init_pos_.z,title_,grids_.size()};
  }
  ...
  for(int z = 0; z < static_cast<int>(grids_.size()); ++z) {
    ...
    if(id == -1 || !dantares_.IsMap(id)) {
      throw CybelError{"Failed to add map grid [{},{}:{}] to Dantares.",z,id,title_};
    }
    if(!dantares_.SetCurrentMap(id)) {
      throw CybelError{"Failed to make map grid [{},{}:{}] current in Dantares.",z,id,title_};
    }
    ...
    for(Pos2i pos{0,0}; pos.y < size.h; ++pos.y) {
      for(pos.x = 0; pos.x < size.w; ++pos.x) {
        const Space& space = grid->unsafe_space(pos);
        update_bridge_space(pos.x,pos.y,space.type());
      }
    }

    set_textures_(dantares_,z,id);

    // Must be called after setting the textures.
    if(!dantares_.GenerateMap()) {
      throw CybelError{"Failed to generate map grid [{},{}:{}] in Dantares.",z,id,title_};
    }
  }
  ...
  if(!change_grid(z,true)) {
    throw CybelError{"Failed to change to map grid [{},{}:{}] in Dantares.",z,id,title_};
  }
  if(!dantares_.SetPlayerPosition(player_init_pos_.x,player_init_pos_.y,dan_facing)) {
    throw CybelError{"Failed to set player pos [{}:({},{})] for map grid [{},{}:{}] in Dantares.",
                     dan_facing,player_init_pos_.x,player_init_pos_.y,z,id,title_};
  }
...
void DantaresMap::on_gpu_context_restore() {
  ...
  if(player_pos.z < 0 || player_pos.z >= static_cast<int>(grid_ids_.size())) {
    throw CybelError{"Invalid player Z [{}] for map [{}] of size [{}].",
                     player_pos.z,title_,grid_ids_.size()};
  }
  ...
  for(int z = 0; z < static_cast<int>(grid_ids_.size()); ++z) {
    ...
    if(id == -1 || !dantares_.IsMap(id)) {
      throw CybelError{"Invalid map grid [{},{}:{}] in Dantares.",z,id,title_};
    }
    if(!dantares_.SetCurrentMap(id)) {
      throw CybelError{"Failed to make map grid [{},{}:{}] current in Dantares.",z,id,title_};
    }
  ...
  if(!dantares_.SetPlayerPosition(player_pos.x,player_pos.y)) {
    throw CybelError{"Failed to set player pos ({},{}) for map grid [{},{}:{}] in Dantares.",
                     player_pos.x,player_pos.y,z,id,title_};
  }
}
