...
PlayMenuScene::PlayMenuScene(GameSession& sesh,const AssetMan& assets)
  : sesh_{sesh} {
  glob_maps(assets);
}

void PlayMenuScene::on_scene_input_event(input_id_t input_id,SceneContext& ctx) {
    ...
    case InputAction::kSelect:
      if(map_opt_index_ == 1) {
        ...
        if(map_opts_.size() <= kNonMapOptCount) {
          CybelEngine::show_error("No map to select.");
        } else {
          select_map();
...
void PlayMenuScene::draw_scene(Renderer& ren,[[maybe_unused]] SceneContext& ctx) {
...
void PlayMenuScene::glob_maps(const AssetMan& assets) {
  ...
  sesh_.assets.glob_maps_meta(assets,[&](const auto& group,const auto& map_file,const auto& map) {
    MapOption opt{};
  ...
  if(map_opts_.size() <= kNonMapOptCount) {
    CybelEngine::show_error("No Maps were found/loaded in the sub folders of the Maps folder `{}`.",
...
void PlayMenuScene::prev_map_opt_group() {
  if(map_opts_.empty()) { return; }
...
void PlayMenuScene::next_map_opt_group() {
  if(map_opts_.empty()) { return; }
...
void PlayMenuScene::select_map_opt(int index,bool wrap) {
  if(map_opts_.empty()) {
    map_opt_index_ = 0;
    return;
  }
...
PlayMenuScene::MapOption::MapOption(std::string_view text)
  : text{text} {}
