...
void GameOverlay::update_logic(const FrameDelta& delta,const SceneContext& ctx) {
  flash_tween_.flash(delta);
  fade_tween_.fade(delta);
  game_over_tween_.fade(delta);
...
void GameOverlay::draw(Renderer& ren,const SceneContext& ctx) {
  if(flash_tween_.is_playing) {
    ...
    ren.wrap_color(flash_color_,[&] {
      ren.draw_quad(Pos3i{0,0,0},ctx.view.size);
    });
  }
...
void GameOverlay::draw_map_info(Renderer& ren,const SceneContext& ctx) {
  ...
  sesh_.assets.font_renderer().wrap(ren,ctx,Pos3i{},[&](auto& font) {
    ...
    const Pos3i pos{
      std::max((ren.view().target_size.w - true_size.w) >> 1,0),
      std::max((ren.view().target_size.h - true_size.h) >> 1,0),
      0
    };
