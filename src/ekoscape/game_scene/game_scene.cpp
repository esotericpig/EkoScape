...
#include "ekoscape/game_scene/dantares_renderer.h"
#include "ekoscape/maps/dantares_map.h"
...
void GameScene::update_scene_logic(const FrameDelta& delta,SceneContext& ctx) {
  dantares_->UpdateDeltaTime(delta.time);

  if(game_phase_ == GamePhase::kShowMapInfo && map_info_timer_.peek() >= kMapInfoDuration) {
    game_phase_ = GamePhase::kPlay;
    robot_move_time_ += delta.duration;

    speedrun_timer_.start();
  }
  if(game_phase_ != GamePhase::kShowMapInfo) {
    update_player(delta,ctx);
    update_robots(delta);
  }

  update_mods(delta,ctx);
}

void GameScene::update_player(const FrameDelta& delta,const SceneContext& ctx) {
  if(game_phase_ == GamePhase::kGameOver) { return; }

  player_warp_ticker_.tick(delta);

  if(player_fruit_ticker_.tick(delta)) {
...
void GameScene::update_robots(const FrameDelta& delta) {
...
void GameScene::move_robots(const FrameDelta& delta) {
  ...
  // Reset the move time.
  robot_move_time_ = map_->robot_delay() + delta.duration;
}
...
void GameScene::update_mods(const FrameDelta& delta,const SceneContext& ctx) {
  ...
  hud_->update_logic(delta);
  overlay_->update_logic(delta,ctx);
}
...
void GameScene::set_space_textures(AssetMan& assets,SpaceType type,std::optional<asset_id_t> ceiling_id,
  ...
  const char space_id = SpaceTypes::value_of(type);
