...
void StarSys::init(const Viewport& view,bool is_flying) {
  is_flying_ = is_flying;
  stars_.resize(100);

  for(auto& star : stars_) {
    birth_star(star,view);
...
void StarSys::birth_star(Particle& star,const Viewport& view) {
  auto& r = Rando::it();

  if(is_flying_) {
    star.lifespan = r.rand_float(0.0f,2.0f);
    star.pos.x = static_cast<float>(view.target_size.w) / 2.0f;
    star.pos.y = static_cast<float>(view.target_size.h) / 2.0f;
    ...
    star.pos.x = static_cast<float>(r.rand_int(view.target_size.w));
    star.pos.y = static_cast<float>(r.rand_int(view.target_size.h));
...
void StarSys::update(const FrameDelta& delta,const Viewport& view) {
  for(auto& star : stars_) {
    if(star.is_alive()) {
      star.age_by(delta.time);
